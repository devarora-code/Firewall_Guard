...
function isBenignConsoleNoiseEntry(command) {
    ...
    return /react-i18next::\s*useTranslation:\s*You will need to pass in an i18next instance by using initReactI18next/i.test(combined) ||
        ...
        /getItem\s*->\s*Error in getting item from indexedDB:?.*Object store\s+["'`A-Za-z0-9_ -]+\s+does not exist/i.test(combined) ||
        /Object store\s+["'`A-Za-z0-9_ -]+\s+does not exist/i.test(combined) ||
        /\[DEPRECATED\]\s*Default export is deprecated\.\s*Instead use\s*`?import\s*\{\s*create\s*\}\s*from\s*['"]zustand['"]`?/i.test(combined) ||
        /\[DEPRECATED\]\s*`?getStorage`?,\s*`?serialize`?\s*and\s*`?deserialize`?\s*options are deprecated\.\s*Use\s*`?storage`?\s*option instead\./i.test(combined) ||
        /\[COMPLETION\]\s*Failed all attempts to invalidate conversation tree/i.test(combined);
}
