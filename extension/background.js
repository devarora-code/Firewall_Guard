const DEFAULT_API_BASE = 'http://localhost:5000/api'; // Backend server connection
const DEFAULT_LOCAL_ENGINE = 'http://localhost:7000/api'; // Local engine connection
...
function isDevTunnelEndpoint(value) {
    try {
        return new URL(value).hostname.toLowerCase().includes('devtunnels.ms');
    } catch (error) {
        return false;
    }
}
...
function getEndpointMode(config) {
    ...
    return isDevTunnelEndpoint(effectiveConfig.backendApiBase) || isDevTunnelEndpoint(effectiveConfig.localEngineApiBase)
        ? 'dev_tunnel'
        : 'localhost';
}
...
function validateEndpointConfigInput(config) {
    ...
    assertHttpUrl(candidate.backendApiBase, 'Backend API endpoint');
    assertHttpUrl(candidate.localEngineApiBase, 'Local engine endpoint');
}
...
async function loadEndpointConfig() {
    const result = await chrome.storage.local.get([ENDPOINT_CONFIG_KEY]);
    const storedConfig = result[ENDPOINT_CONFIG_KEY];
    const config = await applyEndpointConfig(storedConfig || DEFAULT_ENDPOINT_CONFIG);

    if (!storedConfig) {
        await chrome.storage.local.set({ [ENDPOINT_CONFIG_KEY]: config });
    }
...
async function updateLocalEngineSnapshot(options = {}) {
    ...
    try {
        const [statusResponse, rulesResponse] = await Promise.all([
            fetch(`${LOCAL_ENGINE}/status`),
            fetch(`${LOCAL_ENGINE}/rules/current`)
        ]);
...
async function fetchBackendRulesSnapshot() {
    ...
    const endpoints = [
        { url: `${API_BASE}/rules/update`, options: { method: 'GET' } },
        { url: `${API_BASE}/rules/current`, options: { method: 'GET' } },
        { url: `${API_BASE}/rules`, options: { method: 'GET' } }
    ];
    let lastError = null;
...
async function saveConsoleFirewallPolicyToBackend(policy) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/console-policy`, {
            method: 'POST',
...
const endpointConfigReady = loadEndpointConfig().catch((error) => {
    console.warn('[Config] Failed to load saved endpoints, using localhost defaults:', error.message);
    ...
    try {
        switch (message.action) {
            ...
            case 'resetEndpointConfig':
                try {
                    const resetConfig = await applyEndpointConfig(DEFAULT_ENDPOINT_CONFIG, { persist: true });
                    backendConnected = false;
                    const connected = await checkBackendConnection();
                    const localEngineSnapshot = await updateLocalEngineSnapshot({ skipHistory: true });
            ...
            case 'getBackendStatus':
                ...
                sendResponse({
                    ...
                    config: getCurrentEndpointConfig(),
                    mode: getEndpointMode(getCurrentEndpointConfig())
                });
                break;
            case 'checkUpdate':
                try {
    ...
    try {
        ...
        if (backendConnected) {
            ...
            chrome.notifications.create({
                ...
                message: 'Working in local mode - backend unavailable'
            });
        } else {
...
async function checkRuleUpdates(options = {}) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/rules/check-update`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function updateLocalEngine(rules) {
    ...
    try {
        const response = await fetch(`${LOCAL_ENGINE}/rules/update`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
